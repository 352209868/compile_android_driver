...
// 用extern申明链接符号，去引用下面汇编标签符号的地址，运行时把下面汇编nop修改为目标指令
extern char bp_orig_insn_slot[];
extern char wp_orig_insn_slot[];
extern char bp_return_slot[];
extern char wp_return_slot[];
...
__attribute__((used)) static void work_trampoline_breakpoint(unsigned long unused, unsigned long esr, struct pt_regs *regs)
{
    ...
    /*
     * 步过策略:
     *   - PC 被修改 (redirect_pc != 0, 无痕 Hook 模式): 保持 BCR 启用,
     *     下次 CPU 到达该地址时继续触发.
     *   - PC 未修改 (调试模式): 关闭 BCR 启用位做步过,
     *     依赖线程调度回调重新安装断点.
     */
    if (!cfg || regs->pc == saved_pc) {
...
__attribute__((naked, __noinline__)) static int trampoline_breakpoint(unsigned long unused, unsigned long esr, struct pt_regs *regs)
{
    asm volatile(
        ...
        // 初始是 nop，安装 hook 时 patch 成 orig_bp_insn（执行被 hook 函数的第一条指令）
        ".global bp_orig_insn_slot\n"
        "bp_orig_insn_slot:\n"
        ".inst 0xd503201f\n"

        // 初始是 nop，安装 hook 时 patch 成 B <breakpoint_handler+4>
        // 使用直接 B 指令而非 br x16，避免 BTI 检查导致死机
        ".global bp_return_slot\n"
        "bp_return_slot:\n"
        ".inst 0xd503201f\n"
        :
        :
...
__attribute__((naked, __noinline__)) static int trampoline_watchpoint(unsigned long addr, unsigned long esr, struct pt_regs *regs)
{
    asm volatile(
        ...
        // 初始是 nop，安装 hook 时 patch 成 orig_wp_insn（执行被 hook 函数的第一条指令）
        ".global wp_orig_insn_slot\n"
        "wp_orig_insn_slot:\n"
        ".inst 0xd503201f\n"

        // 初始是 nop，安装 hook 时 patch 成 B <watchpoint_handler+4>
        // 使用直接 B 指令而非 br x16，避免 BTI 检查导致死机
        ".global wp_return_slot\n"
        "wp_return_slot:\n"
        ".inst 0xd503201f\n"
        :
        :
...
int hw_breakpoint_hook_install(void)
{
    ...
    //  把 return_slot 的 nop 改成 B <handler+4>，避免 br x16 触发 BTI 异常
    {
        u32 bp_ret = arm64_encode_branch((unsigned long)bp_return_slot, breakpoint_handler + 4);
        u32 wp_ret = arm64_encode_branch((unsigned long)wp_return_slot, watchpoint_handler + 4);
        if (!bp_ret || !wp_ret)
        {
            pr_debug("[driver] B return branch out of range\n");
            fn_aarch64_insn_patch_text_nosync((void *)wp_orig_insn_slot, 0xd503201f);
            fn_aarch64_insn_patch_text_nosync((void *)bp_orig_insn_slot, 0xd503201f);
        ...
        fn_aarch64_insn_patch_text_nosync((void *)bp_return_slot, bp_ret);
        fn_aarch64_insn_patch_text_nosync((void *)wp_return_slot, wp_ret);
    }
